...
describe('GET /dashboard (Integration)', () => {
  ...
  it('non-admin without user_manage permission does NOT see Settings tab', async () => {
    ...
    // Assert against the rendered BODY only: the always-included dashboard client
    // JS references [data-section="settings"] as a selector string, so a raw
    // res.text match would false-positive. Strip <script> blocks first.
    const body = res.text.replace(/<script[\s\S]*?<\/script>/gi, '');
    expect(body).not.toContain('data-tab="settings"');   // tab button gated out
    expect(body).not.toContain('data-section="settings"'); // section markup gated out
  });
